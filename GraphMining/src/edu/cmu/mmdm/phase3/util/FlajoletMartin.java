...
/**
 * Description: FM algorithm, used in radii computation 
 */
public class FlajoletMartin {

	private static final double PHI = 0.77351;

	public static int[] getKBitString(int number, int k){
		int[] result = new int[k];
		for(int i = 0; i < k; ++ i){
			result[i] = FMBitmask.create_random_bm(number, 32);
		}
		return result;
	}

	public static int getNum(int[] bitstrings) {
		double avgBitPos = 0.0;
		for (int i = 0; i < bitstrings.length; ++i)
			avgBitPos += (double) FMBitmask
					.find_least_zero_pos((long) bitstrings[i]);
		avgBitPos /= bitstrings.length;
		return (int)(Math.pow(2, avgBitPos) / PHI);
	}

	public static void main(String[] args) {
		int[] bitstr = FlajoletMartin.getKBitString(1000, 32);
		for(int i = 0; i < bitstr.length; ++ i){
			//System.out.println(FMBitmask.find_least_zero_pos(bitstr[i]));
		}
		System.out.println(FlajoletMartin.getNum(bitstr));
	}
}

class FMBitmask {
	// generate K replicated bitmasks for one node
	public static String generate_bitmask(int number_node, int K,
			int encode_bitmask) {
		int i;
		int size_bitmask = 32;
		...
		for (i = 0; i < K; i++) {
			if (encode_bitmask == 1)
				...
				bitmask = bitmask
						+ " "
						+ Integer.toHexString(create_random_bm(number_node,
								size_bitmask));
		}

		if (encode_bitmask == 1) {
			String encoded_bitmask = BitShuffleCoder.encode_bitmasks(bm_array,
					K);
	...
	// Create a Flajolet-Martin bitstring. The maximum number of nodes is
	// currently 4 billion.
...
/**
 * Description: Bit Shuffle Encoder and Decoder
 */
class BitShuffleCoder {
	...
	private static void fill_result(int[] result, int K, int cur_value,
			int cumulated_value) {
		int i, j;
